...
interface BiometricData {
  ...
  touchEvents?: Array<{ pressure: number; velocity: number }>;
  deviceMotion?: { roll: number; pitch: number; yaw: number };
}
...
export default async function handler(
  ...
  try {
    const body = req.body as BiometricData;
    const { userId, sessionId, accelerometerData, touchEvents } = body;
    ...
    const gaitConfidence = Math.max(0, Math.min(100,
      GaitAnalyzer.analyze(accelerometerData || [])
    ));
    testResults.push({
    ...
    const touchConfidence = Math.max(0, Math.min(100,
      TouchDynamicsAnalyzer.analyze(touchEvents || [])
    ));
    testResults.push({
    ...
    const handMotionConfidence = Math.max(0, Math.min(100,
      HandMotionAnalyzer.analyze()
    ));
    testResults.push({
    ...
    const behavioralConfidence = Math.max(0, Math.min(100,
      BehavioralPatternAnalyzer.analyze()
    ));
    testResults.push({
	...
	console.log('=== BEFORE test_results insert ===');
    	console.log('userId:', userId);
    	console.log('testResults length:', testResults.length);
    	console.log('First test:', testResults[0]);
	...
	console.log('=== AFTER test_results insert ===');
